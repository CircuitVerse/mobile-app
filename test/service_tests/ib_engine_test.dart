  ...
  group('IbEngineService Test -', () {
    ...
    group('getChapters -', () {
      test('When called and returns success response', () async {
        ...
        expect(
          _actualResult[0].items?.length,
          _expectedResult[0].items?.length,
        );
        expect(_actualResult[0].items?[0].id, _expectedResult[0].items?[0].id);
        expect(
          _actualResult[0].items?[0].value,
          _expectedResult[0].items?[0].value,
        );
        expect(
          _actualResult[0].items?[0].prev?.id,
          _expectedResult[0].items?[0].prev?.id,
        );
        expect(
          _actualResult[0].items?[0].next?.id,
      ...
      test('When called and throws Failure', () async {
        ...
        expect(
          () async => await _ibEngine.getChapters(),
    ...
    group('getPageData -', () {
      test('When Home page called and returns success response', () async {
        ...
        when(
          _ibApi.fetchRawPageData(id: mockIbRawPageData1['name']),
        ).thenAnswer(
        ...
        expect(
          _actualResult.content?[0].content,
          _expectedResult.content?[0].content,
        );
      });

      test('When a regular page called and returns success response', () async {
        var mockDataFile = File(
          'test/setup/test_data/contributing_guidelines.json',
        );
        Map<String, dynamic> mockIbRawPageData2 = jsonDecode(
          await mockDataFile.readAsString(),
        );
        ...
        var _expectedResult = IbPageData(
          ...
          tableOfContents: [
            IbTocItem(
              ...
              items: [
                IbTocItem(leading: 'a.', content: 'Revision history'),
                IbTocItem(leading: 'b.', content: 'Purpose of the guidelines'),
                IbTocItem(leading: 'c.', content: 'Acknowledgements'),
              ],
            ),
            IbTocItem(leading: '2.', content: 'Workflow'),
            IbTocItem(
              leading: '3.',
              content: 'Licensing',
              items: [
            ...
            IbTocItem(leading: '4.', content: 'Proposing a contribution'),
            IbTocItem(leading: '5.', content: 'Editing existing content'),
            IbTocItem(leading: '6.', content: 'Writing content'),
            IbTocItem(leading: '7.', content: 'Style manual'),
            IbTocItem(leading: '8.', content: 'Templates and examples'),
            IbTocItem(
              leading: '9.',
              content:
                  'Code of conduct, interacting with the community / etiquette',
            ),
            IbTocItem(leading: '10.', content: 'Tools'),
          ],
        );
        ...
        when(
          _ibApi.fetchRawPageData(id: mockIbRawPageData2['path']),
        ).thenAnswer(
          (_) => Future.value(IbRawPageData.fromJson(mockIbRawPageData2)),
        );
      ...
      test('When called and throws Failure', () async {
        ...
        when(
          _ibApi.fetchRawPageData(id: 'index.md'),
        ).thenAnswer((_) => throw Exception('Service Unavailable'));

        var _ibEngine = IbEngineServiceImpl();

        expect(
          () async => await _ibEngine.getPageData(),
          throwsA(isInstanceOf<Failure>()),
        );
      });
    });
    ...
    group('getHtmlInteraction -', () {
      test('When binary embed is called and returns success response', () async {
...
''';
        ...
        expect(
          _expectedResult[0].answers.length,
          _actualResult[0].answers.length,
        );
        expect(_expectedResult[0].answers[0], _actualResult[0].answers[0]);
        expect(
          _expectedResult[0].choices.length,
          _actualResult[0].choices.length,
        );
        ...
        expect(
          _expectedResult[1].answers.length,
          _actualResult[1].answers.length,
        );
        expect(_expectedResult[1].answers[0], _actualResult[1].answers[0]);
        expect(
          _expectedResult[1].choices.length,
          _actualResult[1].choices.length,
        );
        ...
        expect(
          _expectedResult[2].answers.length,
          _actualResult[2].answers.length,
        );
        expect(_expectedResult[2].answers[0], _actualResult[2].answers[0]);
        expect(
          _expectedResult[2].choices.length,
          _actualResult[2].choices.length,
        );
    ...
    group('getSlug -', () {
      test('Type 1 - Slug', () {
        ...
        expect(
          IbEngineService.getSlug('Representing real numbers'),
      ...
      test('Type 2 - Slug', () {
        expect(
          IbEngineService.getSlug('The IEEE 754 Standard'),
