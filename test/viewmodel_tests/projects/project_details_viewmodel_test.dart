...
void main() {
  group('ProjectDetailsViewModelTest -', () {
    ...
    var _addedCollaborators = AddCollaboratorsResponse.fromJson(
      mockAddCollaboratorsResponse,
    );
    ...
    group('fetchGroupDetails -', () {
      test('When called & service returns success response', () async {
        ...
        when(
          _mockProjectsApi.getProjectDetails('1'),
        ...
        expect(
          _model.stateFor(_model.FETCH_PROJECT_DETAILS),
          ViewState.Success,
        );
    ...
    group('addCollaborators -', () {
      test('When called & service returns success response', () async {
        ...
        when(
          _mockCollaboratorsApi.addCollaborators(
            '1',
            'test@test.com,existing@test.com,invalid@test.com',
          ),
        ).thenAnswer((_) => Future.value(_addedCollaborators));

        when(
          _mockCollaboratorsApi.fetchProjectCollaborators('1'),
    ...
    group('deleteGroupMember -', () {
      test('When called & service returns success/true response', () async {
        var _mockCollaboratorsApi = getAndRegisterCollaboratorsApiMock();
        when(
          _mockCollaboratorsApi.deleteCollaborator('1', '1'),
    ...
    group('forkProject -', () {
      test('When called & service returns success response', () async {
        var _mockProjectsApi = getAndRegisterProjectsApiMock();
        when(
          _mockProjectsApi.forkProject('1'),
        ).thenAnswer((_) => Future.value(_project));
    ...
    group('toggleStarProject -', () {
      test('When called & service returns "Starred" response', () async {
        var _mockProjectsApi = getAndRegisterProjectsApiMock();
        when(
          _mockProjectsApi.toggleStarProject('1'),
    ...
    group('deleteProject -', () {
      test('When called & service returns success/true response', () async {
        var _mockProjectsApi = getAndRegisterProjectsApiMock();
        when(
          _mockProjectsApi.deleteProject('1'),
        ).thenAnswer((_) => Future.value(true));
