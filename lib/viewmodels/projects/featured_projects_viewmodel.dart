...
class FeaturedProjectsViewModel extends BaseModel {
  // ViewState Keys
  String fetchFeaturedProjectsKey = 'fetch_featured_projects';
  ...
  Future fetchFeaturedProjects({int size = 5}) async {
    try {
      if (previousFeaturedProjectsBatch?.links?.next != null) {
        ...
        setStateFor(fetchFeaturedProjectsKey, ViewState.Busy);
      ...
      setStateFor(fetchFeaturedProjectsKey, ViewState.Success);
    } on Failure catch (f) {
      setStateFor(fetchFeaturedProjectsKey, ViewState.Error);
      setErrorMessageFor(fetchFeaturedProjectsKey, f.message);
    }
  }
}
