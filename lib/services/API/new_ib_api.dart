...
abstract class NewIbApi {
  ...
  Future<NewIbTopicData> fetchTopicData(String path);
}
...
class HttpNewIbApi implements NewIbApi {
  ...
  @override
  Future<NewIbTopicData> fetchTopicData(String path) async {
    final url = '$BASE_URL/pages/$path.json';
    ...
    try {
      ...
      if (await _db.isExpired(url)) {
        ...
        return NewIbTopicData.fromJson(jsonResponse as Map<String, dynamic>);
      } else {
        // Return cached data
        final dynamic rawData = await _db.getData(
          DatabaseBox.IB,
        ...
        return NewIbTopicData.fromJson(properData);
      }
    } on FormatException catch (e) {
      throw Failure(Constants.BAD_RESPONSE_FORMAT);
    } on Exception catch (e) {
      throw Failure('Failed to fetch topic data: ${e.toString()}');
    }
  }
}
